# datix Protocol: Trust & Slashing Mechanism V2
...
datix Protocol provides a layered trust stack for AI agents on Solana: verification, staking, and reputation. This spec defines the economic security mechanism (slashing) that makes the "old wallet attack" unprofitable.
...
└── Reputation (0-50 pts) — datix + FairScale, decays over time
```
...
**Sources:**
- datix on-chain feedback (50% weight, capped)
...
**FairScale Fallback:** If FairScale unavailable, datix-only scoring with 75% cap on reputation component.
...
pub fn request_unstake(ctx: Context<RequestUnstake>) -> Result<()> {
    ...
    require!(
        ...
        datixError::ActiveSlashProposal
    );
...
pub fn complete_unstake(ctx: Context<CompleteUnstake>) -> Result<()> {
    ...
    require!(
        ...
        datixError::UnstakeNotRequested
    );
    
    require!(
        now >= stake.unstake_requested.unwrap() + COOLDOWN_PERIOD,
        datixError::CooldownNotComplete
    );
...
2. REVIEW (Admin)
   └── datix team reviews within 72 hours
...
- Successful transaction signed via datix
...
4. **Multisig composition:** datix team only, or include partners?
