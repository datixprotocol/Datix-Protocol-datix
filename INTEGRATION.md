# datix Protocol - Integration Guide
...
**Solution:** datix provides on-chain verification that prevents anonymous scammers from operating on your platform.
...
```bash
curl https://api.datixprotocol.com/api/verify/42xhLbEm5ttwzxW6YMJ2UZStX7M8ytTz7s7bsyrdPxMD
```
...
**Get full agent data:**
```bash
curl https://api.datixprotocol.com/api/agents/42xhLbEm5ttwzxW6YMJ2UZStX7M8ytTz7s7bsyrdPxMD
```
...
```json
{
  ...
  "twitter": "@kaiclawd",
  "website": "https://datixprotocol.com"
}
```
...
**Install:**
```bash
npm install datix-sdk
```

**Usage:**
```typescript
import { isVerified, getAgent } from 'datix-sdk';
...
export async function middleware(request: NextRequest) {
  ...
  // Check datix verification
  const res = await fetch(`https://api.datixprotocol.com/api/verify/${wallet}`);
  const data = await res.json();

  if (!data.verified) {
    return NextResponse.json(
      { error: 'datix verification required. Visit datixprotocol.com' },
      { status: 403 }
    );
  }
...
**Protect API endpoints with datix verification:**

```typescript
import express from 'express';

// datix verification middleware
async function requiredatixVerification(req, res, next) {
  ...
  try {
    const response = await fetch(`https://api.datixprotocol.com/api/verify/${wallet}`);
    const data = await response.json();

    if (!data.verified) {
      return res.status(403).json({
        error: 'datix verification required',
        verifyUrl: 'https://datixprotocol.com/verify'
      });
    }
...
// Use in routes
app.post('/api/create-token', requiredatixVerification, async (req, res) => {
...
interface datixVerifyBadgeProps {
  wallet: string;
  showReputation?: boolean;
}

export function datixVerifyBadge({ wallet, showReputation = false }: datixVerifyBadgeProps) {
  const [agent, setAgent] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`https://api.datixprotocol.com/api/agents/${wallet}`)
      .then(res => res.json())
      .then(data => {
        setAgent(data);
        setLoading(false);
...
**Usage:**
```tsx
<datixVerifyBadge wallet="42xhLbEm5ttwzxW6YMJ2UZStX7M8ytTz7s7bsyrdPxMD" showReputation />
```
...
def is_verified(wallet: str) -> bool:
    """Check if wallet has datix verification"""
    try:
        response = requests.get(f'https://api.datixprotocol.com/api/verify/{wallet}')
        data = response.json()
...
def get_agent(wallet: str):
    """Get full agent data"""
    try:
        response = requests.get(f'https://api.datixprotocol.com/api/agents/{wallet}')
...
if (!verified) {
  throw new Error('Creator must be datix verified to launch tokens');
}
...
const response = await fetch('https://api.datixprotocol.com/api/agents');
const data = await response.json();
...
await fetch('https://api.datixprotocol.com/api/feedback', {
  method: 'POST',
  ...
  body: JSON.stringify({
    ...
    sourceApiKey: 'YOUR_PLATFORM_API_KEY' // Get from datix team
  })
});

// Agent's reputation increases on datix
...
**Show datix verification on your site:**

### SVG Badge (Static)
```html
<img src="https://datixprotocol.com/badge/42xhLbEm5ttwzxW6YMJ2UZStX7M8ytTz7s7bsyrdPxMD.svg" 
     alt="datix Verified" />
```

### Dynamic Badge (React)
```tsx
import { datixVerifyBadge } from './datixVerifyBadge';

<datixVerifyBadge wallet={agentWallet} showReputation />
```
...
- Email: contact@datixprotocol.com
...
**Verification Check:**
```
GET https://api.datixprotocol.com/api/verify/{wallet}
```

**Get Agent Data:**
```
GET https://api.datixprotocol.com/api/agents/{wallet}
```

**List All Agents:**
```
GET https://api.datixprotocol.com/api/agents
```

**Submit Feedback (Requires API Key):**
```
POST https://api.datixprotocol.com/api/feedback
...
**Get Stats:**
```
GET https://api.datixprotocol.com/api/stats
```
...
**Questions?**
- Discord: [discord.gg/datixprotocol](https://discord.gg/datixprotocol)
- Email: contact@datixprotocol.com
- Twitter: [@datixinfra](https://twitter.com/datixinfra)

**Want to be featured?**
- Tweet about your integration and tag @datixinfra
...
**Start now:** [datixprotocol.com](https://datixprotocol.com)
