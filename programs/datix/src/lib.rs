...
security_txt! {
    name: "datix Protocol",
    project_url: "https://datixprotocol.xyz",
    contacts: "email:kaiclawd@outlook.com",
    policy: "https://github.com/kaiclawd/datix/blob/main/security.txt",
    source_code: "https://github.com/kaiclawd/datix",
    auditors: "N/A"
}
...
pub mod datix {
    ...
    pub fn initialize_treasury(ctx: Context<InitializeTreasury>) -> Result<()> {
        require!(ctx.accounts.authority.key() == TREASURY_AUTHORITY, datixError::UnauthorizedAuthority);
    ...
    pub fn register_and_stake(ctx: Context<RegisterAndStake>, metadata_uri: String, stake_lamports: u64) -> Result<()> {
        validate_uri(&metadata_uri)?;
        require!(stake_lamports >= MIN_STAKE_LAMPORTS, datixError::StakeTooLow);
    ...
    pub fn withdraw_fees(ctx: Context<WithdrawFees>, amount: u64) -> Result<()> {
        ...
        require!(treasury.to_account_info().lamports().saturating_sub(amount) >= min_balance, datixError::InsufficientTreasuryBalance);
    ...
    pub fn submit_feedback(ctx: Context<SubmitFeedback>, positive: bool, context: String) -> Result<()> {
        require!(ctx.accounts.reviewer.key() != ctx.accounts.agent_identity.owner && ctx.accounts.reviewer.key() != ctx.accounts.agent_identity.authority, datixError::CannotReviewSelf);
        require!(context.len() <= 500, datixError::ContextTooLong);
    ...
    pub fn stake(ctx: Context<Stake>, stake_lamports: u64) -> Result<()> {
        ...
        let now = Clock::get()?.unix_timestamp;
        let agent = &mut ctx.accounts.agent_identity;
        require!(agent.is_verified, datixError::NotVerified);
        require!(agent.stake_amount == 0, datixError::AlreadyStaked);
    ...
    pub fn add_stake(ctx: Context<AddStake>, additional_lamports: u64) -> Result<()> {
        require!(additional_lamports > 0, datixError::StakeTooLow);
        require!(ctx.accounts.authority.key() == ctx.accounts.agent_identity.authority, datixError::Unauthorized);
        require!(ctx.accounts.agent_stake.amount > 0, datixError::NoActiveStake);
        require!(ctx.accounts.agent_stake.cooldown_until.is_none(), datixError::AlreadyUnstaking);
    ...
    pub fn request_unstake(ctx: Context<RequestUnstake>) -> Result<()> {
        require!(ctx.accounts.authority.key() == ctx.accounts.agent_identity.authority, datixError::Unauthorized);
        let stake = &mut ctx.accounts.agent_stake;
        require!(stake.amount > 0, datixError::NoActiveStake);
        require!(stake.cooldown_until.is_none(), datixError::AlreadyUnstaking);
        require!(!stake.is_slashed, datixError::StakeSlashed);
        let now = Clock::get()?.unix_timestamp;
    ...
    pub fn complete_unstake(ctx: Context<CompleteUnstake>) -> Result<()> {
        ...
        require!(amount > 0, datixError::NoActiveStake);
        require!(ctx.accounts.agent_stake.cooldown_until.is_some(), datixError::UnstakeNotRequested);
        require!(now >= ctx.accounts.agent_stake.cooldown_until.unwrap(), datixError::CooldownNotComplete);
    ...
    pub fn cancel_unstake(ctx: Context<CancelUnstake>) -> Result<()> {
        ...
        let stake = &mut ctx.accounts.agent_stake;
        require!(stake.amount > 0, datixError::NoActiveStake);
        require!(stake.cooldown_until.is_some(), datixError::UnstakeNotRequested);
        stake.cooldown_until = None;
    ...
    pub fn slash_agent(ctx: Context<SlashAgent>, severity_bps: u16) -> Result<()> {
        require!(ctx.accounts.admin.key() == TREASURY_AUTHORITY, datixError::UnauthorizedAuthority);
        require!(severity_bps <= 10_000, datixError::InvalidSeverity);
        let cur_amount = ctx.accounts.agent_stake.amount;
        require!(cur_amount > 0, datixError::NoActiveStake);
        let slash_amount = (cur_amount as u128 * severity_bps as u128 / 10_000) as u64;
        require!(slash_amount > 0, datixError::NothingToSlash);
    ...
    pub fn submit_anchor(
        ...
        require!(anchor_index == expected_index, datixError::InvalidAnchorRange);
        let expected_start = if agent.last_receipt_seq == 0 { 1 } else { agent.last_receipt_seq + 1 };
        require!(start_seq == expected_start, datixError::InvalidAnchorRange);
        require!(end_seq >= start_seq, datixError::InvalidAnchorRange);
...
pub struct WithdrawFees<'info> {
    ...
    #[account(mut, address = TREASURY_AUTHORITY @ datixError::UnauthorizedAuthority)]
...
pub struct GetVerified<'info> {
    #[account(mut, seeds = [b"agent", agent_identity.owner.as_ref()], bump = agent_identity.bump, constraint = authority.key() == agent_identity.authority @ datixError::Unauthorized)]
    pub agent_identity: Account<'info, AgentIdentity>,
    #[account(mut, seeds = [b"treasury"], bump = treasury.bump)]
    pub treasury: Account<'info, Treasury>,
    #[account(mut)]
...
pub struct LinkWallet<'info> {
    #[account(seeds = [b"agent", agent_identity.owner.as_ref()], bump = agent_identity.bump, constraint = authority.key() == agent_identity.authority @ datixError::Unauthorized)]
    pub agent_identity: Account<'info, AgentIdentity>,
...
pub struct UnlinkWallet<'info> {
    ...
    pub agent_identity: Account<'info, AgentIdentity>,
    #[account(mut, close = caller, seeds = [b"wallet", wallet_link.wallet.as_ref()], bump = wallet_link.bump, constraint = wallet_link.agent_id == agent_identity.key() @ datixError::WalletNotLinked)]
    pub wallet_link: Account<'info, WalletLink>,
    #[account(mut, constraint = caller.key() == agent_identity.authority || caller.key() == wallet_link.wallet @ datixError::Unauthorized)]
...
pub struct TransferAuthority<'info> {
    ...
    pub agent_identity: Account<'info, AgentIdentity>,
    #[account(seeds = [b"wallet", new_authority.key().as_ref()], bump = wallet_link.bump, constraint = wallet_link.agent_id == agent_identity.key() @ datixError::WalletNotLinked)]
    pub wallet_link: Account<'info, WalletLink>,
...
pub struct SponsorVerify<'info> {
    #[account(mut, seeds = [b"agent", agent_identity.owner.as_ref()], bump = agent_identity.bump)]
    pub agent_identity: Account<'info, AgentIdentity>,
    #[account(address = TREASURY_AUTHORITY @ datixError::UnauthorizedAuthority)]
    pub authority: Signer<'info>,
}
